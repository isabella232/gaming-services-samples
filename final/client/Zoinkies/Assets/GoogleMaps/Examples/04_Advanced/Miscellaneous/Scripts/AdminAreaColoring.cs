...
namespace Google.Maps.Examples {
  ...
  public sealed class AdminAreaColoring : MonoBehaviour {
    ...
    private void Start() {
      ...
      Material adminAreaMaterial =
          BaseMapMaterialUtils.CreateUniformColoredMaterial(AdminAreaColor);
      ...
      mapsService.Events.RegionEvents.DidCreate.AddListener(args => {
        if (borderMaterial == null) {
          // Set up the border material based on the prefecture material, but change the render
          // queue so that it renders on top.
        ...
        Extruder.AddAreaExternalOutline(
            args.GameObject, borderMaterial, args.MapFeature.Shape, BorderWidth);
      });

      // Don't display water.
      mapsService.Events.AreaWaterEvents.WillCreate.AddListener(args => { args.Cancel = true; });
      ...
      mapsService.MakeMapLoadRegion()
          ...
          .Load(ExampleDefaults.DefaultGameObjectOptions); // Zoom level 5.
    }
  }
}
