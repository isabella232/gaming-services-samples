...
namespace Google.Maps.Examples.Shared {
  ...
  public sealed class WorldspaceTexturingExample : MonoBehaviour {
    ...
    /// <summary>
    /// Id of the vector property within all given world space textured <see cref="Material"/>s,
    /// used for offsetting the world space texture coordinates.
    /// </summary>
    /// <remarks>
    ...
    [Tooltip(
        "Material to apply to buildings. If this is a world space textured Material, its Shader " +
        "must have an _Offset Vector, as this will be used for updating the origin of the world.")]
    public Material Buildings;

    [Tooltip(
        "Material to apply to road segments. If this is a world space textured Material, its " +
        "Shader must have an _Offset Vector, as this will be used for updating the origin of the " +
        "world.")]
    public Material Roads;

    [Tooltip(
        "Material to apply to the borders of road segments and buildings. If this is a " +
    ...
    [Tooltip(
        "Material to apply to ground regions. If this is a world space textured Material, its " +
        "Shader must have an _Offset Vector, as this will be used for updating the origin of the " +
        "world.")]
    public Material Ground;

    [Tooltip(
        "Material to apply to water. If this is a world space textured Material, its Shader " +
        "must have an _Offset Vector, as this will be used for updating the origin of the world.")]
    ...
    private void Start() {
      ...
      // Store all Materials that are to be updated. This function only stores Materials that have
      // an _Offset Vector, which will be used to offset the Material's world space coordinates to
      // align to the world's moved Floating Origin.
      ...
      ExtrudedStructureStyle extrudedStructureStyle =
          new ExtrudedStructureStyle.Builder { WallMaterial = Buildings, RoofMaterial = Buildings }
              .Build();

      ModeledStructureStyle modeledStructureStyle =
          new ModeledStructureStyle.Builder { Material = Buildings }.Build();

      SegmentStyle roadsStyle =
          new SegmentStyle
              .Builder {
                Material = Roads, BorderMaterial = Borders, Width = 7.0f, BorderWidth = BorderWidth
              }
              .Build();

      RegionStyle groundStyle = new RegionStyle.Builder { FillMaterial = Ground }.Build();

      AreaWaterStyle areaWaterStyle = new AreaWaterStyle.Builder { FillMaterial = Water }.Build();

      LineWaterStyle lineWaterStyle =
          new LineWaterStyle.Builder { Material = Water, Width = 7.0f }.Build();
      ...
      // resolve z-fighting issues), that these new cloned materials are added to the list of
      // managed Materials.
    ...
    private void TryAddMaterial(Material material) {
      // If this Material does not have the required _Offset Vector, skip adding it, as this Vector
      // is used to update the Floating Origin on world space textured Materials. If the Material
      // doesn't have this Vector, it is either not a world space textured Material, or it is not
      // set up so that this script can interact with it.
      ...
      // stored as a HashSet, adding will be skipped if this specific Material has already been
      // added.
      WorldspaceMaterials.Add(material);
    }

    /// <summary>
    /// Add any <see cref="Material"/>s from the supplied <see cref="GameObject"/> and its children
    /// if appropriate.
    /// </summary>
    /// <remarks>
    ...
    /// <see cref="GameObject"/> to try to find one or more <see
    /// cref="MeshRenderer.sharedMaterial"/>s under.
    /// </param>
    private void TryAddMaterialFrom(GameObject geoGameObject) {
      ...
      // If given GameObject has any MeshRenderers, see if their shared Materials need to be
      // managed.
    ...
    /// <summary>Init the offset of all managed <see cref="WorldspaceMaterials"/>.</summary>
    ...
    private void UpdateWorldspaceMaterialOffsets(Vector3 delta) {
      ...
      // textured Materials stay aligned to their geometry, even after said geometry is moved by
      // Maps Service when the Floating Origin is recentered.
      Offset += delta;
    ...
    private bool VerifyParameters() {
      ...
      if (Buildings == null) {
        Debug.LogError(
            ExampleErrors.MissingParameter(this, Buildings, "Buildings", "to assign to buildings"));
      ...
      if (Roads == null) {
        Debug.LogError(
            ExampleErrors.MissingParameter(this, Roads, "Roads", "to assign to road segments"));
      ...
      if (Ground == null) {
        Debug.LogError(
            ExampleErrors.MissingParameter(this, Ground, "Ground", "to assign to ground regions"));
      ...
      if (!Buildings.HasProperty(MaterialOffsetProperty) &&
        ...
        Debug.LogErrorFormat(
            "No '{0}' property found on any of the Shaders of any of the Materials " +
                "given to {1}.{2}.\n{2} is used to adjust the offset of this Material's world " +
                "space texture coordinates, but given none of the given Materials had a '{0}' " +
                "property to use for this purpose, {1}.{2} will be disabled.",
