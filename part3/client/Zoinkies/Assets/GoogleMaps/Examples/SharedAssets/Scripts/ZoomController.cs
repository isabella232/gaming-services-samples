...
namespace Google.Maps.Examples.Shared {
  ...
  public sealed class ZoomController : MonoBehaviour {
    ...
    [Tooltip(
        "Distance camera should start at (must be within range of the above Near and Far " +
    ...
    /// <para>
    /// This variable is used instead of <see cref="Camera.main"/> to avoid per-frame searching for
    /// an in scene <see cref="Camera"/> tagged as "Main Camera", instead performing this search
    /// only once and storing the result for future re-use.
    /// </para></remarks>
    public Camera ZoomingCamera {
      get {
        // If we have already found the main Camera (and it is has not yet been destroyed), return
        // it now.
    ...
    private void Awake() {
      ...
      if (StartDistance < NearDistance || StartDistance > FarDistance) {
        Debug.LogError(ExampleErrors.OutsideRange(
            this, StartDistance, "Start Distance", NearDistance, FarDistance));
    ...
    /// <remarks>
    /// Distance to zoom to (assumed to be positive and within range of <see cref="NearDistance"/>
    /// and <see cref="FarDistance"/>.
    /// </remarks>
    ...
    private bool VerifyRange() {
      ...
      if (FarDistance <= NearDistance) {
        Debug.LogError(ExampleErrors.NotGreaterThan(
            this, FarDistance, "Far Distance", NearDistance, "Near Distance"));
