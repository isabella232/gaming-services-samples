...
namespace Google.Maps.Examples {
  ...
  public class TerrainExample : MonoBehaviour {
    ...
    private IEnumerator PaintControlTexture(AlphaMapsNeedPaintArgs args) {
      ...
      args.Terrain.terrainData.CopyActiveRenderTextureToTexture(
          TerrainData.AlphamapTextureName, 0,
          new RectInt(0, 0, temporaryRenderTexture.width, temporaryRenderTexture.height),
          Vector2Int.zero, true);
      args.Terrain.terrainData.SetBaseMapDirty();
#else
      // May be slower than the above method.
    ...
    /// <summary>
    /// Handle <see cref="TerrainEvents.FeatureMaskTextureCreated"/> by setting the
    /// anti-aliasing level to 4x.
    /// </summary>
    /// <param name="args">Event arguments.</param>
    public void HandleFeatureMaskTextureCreated(FeatureMaskTextureCreatedArgs args) {
      args.FeatureMaskRenderTexture.antiAliasing = 4;
    }
  }
}
