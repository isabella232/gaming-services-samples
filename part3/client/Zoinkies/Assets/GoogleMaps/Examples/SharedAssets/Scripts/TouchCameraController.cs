...
namespace Google.Maps.Examples.Shared {
  ...
  /// with rotation occuring around the Vector3.Up axis from this point. Zooming is asymptotic, so
  /// it is not possible to zoom through the ground plane.
  /// </para>
  /// <para>
  ...
  public class TouchCameraController : MonoBehaviour {
    ...
    private float TouchAngle(Touch[] touches) {
      if (touches.Length != 2) {
        throw new InvalidOperationException(
            "TouchAngle must be called with exactly 2 touch points");
      }
    ...
    /// <summary>
    /// Process touch state: if a single finger is touching the screen, try to process a drag; if
    /// two fingers, try to process zoom and rotation gestures.
    /// </summary>
    void Update() {
    ...
    private void UpdateTwoFingerZoom(Touch[] touches) {
      ...
      if (touches[0].phase == TouchPhase.Began || touches[1].phase == TouchPhase.Began ||
          !Zooming || touches[0].fingerId != PrevFingerId0 ||
          touches[1].fingerId != PrevFingerId1) {
      ...
      // If either finger has moved, use the inverse ratio of the finger separation distance from
      // this invocation and the previous invocation to scale the camera distance from the ground
      // plane. This achieves asymptotic zooming towards the ground plane.
      if (touches[0].phase == TouchPhase.Moved && touches[1].phase == TouchPhase.Moved) {
        ...
        float currentGroundPlaneDistance =
            Vector3.Distance(MainCamera.transform.position, groundPos);
    ...
    private void UpdateTwoFingerRotate(Touch[] touches) {
      ...
      if (touches[0].phase == TouchPhase.Began || touches[1].phase == TouchPhase.Began ||
          !Rotating || touches[0].fingerId != PrevFingerId0 ||
          touches[1].fingerId != PrevFingerId1) {
      ...
      // touch points and use this as the angle of rotation around the vertical axis intersecting
      // the ground plane target point of the camera line of sight.
      if (touches[0].phase == TouchPhase.Moved && touches[1].phase == TouchPhase.Moved) {
