...
public class GameUtils {
  ...
  public PlayerData CreateNewUser() {
    ...
    // TESTING only
    //inventory.add(new Item(GameConstants.GOLD_KEY, 100));
    //inventory.add(new Item(GameConstants.DIAMOND_KEY, 100));
   ...
   * @throws Exception
   */
  private void CheckLocationStatus(String Id, String PlaceId) throws Exception{

    WorldData worldData = WorldService.GetWorldData(Id);
    if (!worldData.getLocations().containsKey(PlaceId)) {
      throw new Exception("Place Id " + PlaceId + " not found!");
    }
    SpawnLocation location = worldData.getLocations().get(PlaceId);
    if (!location.active) {
      ...
      if (t.compareTo(Instant.now()) > 0) {
        // Still respawning
        throw new LocationStillRespawningException("Location " + PlaceId + " is still respawning!");
      } else {
   ...
   * @throws Exception
   */
  private void StartRespawiningLocation(String ItemId, String UserId, String PlaceId, WorldData WorldData) throws Exception {
  ...
  public BattleSummaryData GetBattleSummaryData(String Id, String PlaceId, Boolean Winner) throws Exception {
    WorldData worldData = WorldService.GetWorldData(Id);
    if (!worldData.getLocations().containsKey(PlaceId)) {
      throw new Exception("Place Id " + PlaceId + " not found!");
    }
    ...
    if (location.object_type_id.equals(GameConstants.MINION)
      ...
      CheckLocationStatus(Id,PlaceId);
      ...
      if (Winner) { // Player wins
        ...
        if (freedLeaders.size() > 0 && freedLeaders.get(0).getQuantity() >= GameConstants.FREED_LEADERS_TO_WIN) {
      ...
      data.rewards.setPlaceId(PlaceId);
   ...
   * @throws Exception
   */
  public BattleData GetBattleData(String Id, String PlaceId) throws Exception {
    WorldData worldData = WorldService.GetWorldData(Id);
    if (!worldData.getLocations().containsKey(PlaceId)) {
      throw new Exception("Place Id " + PlaceId + " not found!");
    }
    ...
    // General/towers - no regeneration - grant leaders - tower disappears afterwards - requires diamond keys
  ...
  public EnergyData GetEnergyStationData(String Id, String PlaceId) throws Exception {
    ...
    StartRespawiningLocation(GameConstants.ENERGY_STATION, Id, PlaceId, worldData);
   ...
   * @throws Exception
   */
  public RewardsData GetChestRewards(String Id, String PlaceId) throws Exception {
    ...
    if (items.size() > 0
      ...
      // Start respawning
      StartRespawiningLocation(GameConstants.CHEST, Id, PlaceId, worldData);
     // Return response
    } else {
  ...
  public SpawnLocation CreateRandomSpawnLocation(PLLocation loc) throws Exception {
    ...
    else if (loc.plusCode != null && !loc.plusCode.isEmpty()) {
      placeId = loc.plusCode;
    }
    else {
      throw new Exception("Invalid placeId or plusCode found while creating random spawn location!");
    }

    /*if (loc.placeId == null || loc.placeId.isEmpty()) {
      throw new Exception("Invalid placeId found while creating random spawn location!");
    }*/

    if (loc.snappedPoint == null && loc.centerPoint == null) {
      throw new Exception(
          "Invalid Lat Lng coordinates found while creating random spawn location!");
    }

    PLLatLng point = loc.snappedPoint==null?loc.centerPoint:loc.snappedPoint;
