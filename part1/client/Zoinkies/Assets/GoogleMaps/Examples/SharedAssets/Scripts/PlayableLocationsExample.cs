...
namespace Google.Maps.Examples.Shared {
  ...
  public sealed class PlayableLocationsExample : MonoBehaviour {
    ...
    private void Awake() {
      string stateSaveFile = Path.Combine(Application.persistentDataPath, "impressions.dat");
    ...
    private void ReportBadPointExample() {
      PlayableLocationsService.ReportBadPoint(
          "xyz+test",
          PlayableLocationsService.BadLocationReason.NotPedestrianAccessible,
          "in the middle of the lake",
          status => Debug.LogFormat(
              "ReportBadPoint request status: IsError={0}, Error={1}",
              status.IsError,
              status.Error));
    }
    ...
    private void ReportImpressions() {
      ...
      // Next frame the player still sees the same things. Developer is free to call the same
      // methods again. They will not result in any network requests.
      PlayableLocationsService.ReportPresentedPlace(monsterGameObjectType, "a");
      PlayableLocationsService.ReportPresentedPlace(powerUpGameObjectType, "b");
