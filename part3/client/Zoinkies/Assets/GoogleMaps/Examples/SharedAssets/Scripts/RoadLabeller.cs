...
namespace Google.Maps.Examples.Shared {
  ...
  public sealed class RoadLabeller : MonoBehaviour {
    ...
    private void Awake() {
      ...
      if (Canvas == null) {
        Debug.LogError(
            ExampleErrors.MissingParameter(this, Canvas, "Canvas", "to show road names"));
      ...
      if (LabelPrefab == null) {
        Debug.LogError(ExampleErrors.MissingParameter(
            this, LabelPrefab, "LabelPrefab", "to use to display names above roads in scene"));
      }
    }
    ...
    internal void NameRoad(GameObject roadGameObject, Segment roadData) {
      // Get the details of the specific road the Maps SDK for Unity has just created.
